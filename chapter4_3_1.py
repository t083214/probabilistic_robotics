import sys
sys.path.append('./')
from ideal_robot import * # include world and ideal robot
...
class Camera(IdealCamera):
	def __init__(self, env_map,
				 ...
				 direction_rage = (-math.pi/3, math.pi/3),
				 distance_noise_rate = 0.1, direction_noise=math.pi/90,
				 distance_bias_rate_stddev = 0.1, direction_bias_stddev=math.pi/90,
				 phantom_prob = 0.1, phantom_range_x = (-5.0, 5.0), pahntom_range_y = (-5.0, 5.0),
				 oversigit_prob=0.1, occulusion_prob=0.0):

		super().__init__(env_map, distance_range, direction_rage)

		self.distance_noise_rate = distance_noise_rate
		self.direction_noise = direction_noise
		self.distance_bias_rate_stddev = norm.rvs(scale=distance_bias_rate_stddev)
		self.direction_bias = norm.rvs(scale=direction_bias_stddev)

		rx, ry = phantom_range_x, pahntom_range_y
		self.phantom_dist = uniform(loc=(rx[0], ry[0]), scale = (rx[1] - rx[0], ry[1] - ry[0]))
		self.phantom_prob = phantom_prob
		self.oversight_prob = oversigit_prob
	...
	def occulusion(self, relpos):
		if uniform.rvs() < self.occulusion_prob:
			...
			return np.array([ell, relpose[1]]).T
		else:
			return relpos
	...
	def bias(self, relpos):
		return relpos + np.array([relpos[0]*self.distance_bias_rate_stddev, self.direction_bias]).T
